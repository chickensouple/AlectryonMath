...
void TestAngleInterp() {
    T angle1 = 0;
    T angle2 = Angle::HalfPi<T>();

    // basic test checking that halfway points are equal
    CHECK_FLOATING_POINT(T, Angle::angle_interp(angle1, angle2, (T) 0.5), Angle::HalfPi<T>() * 0.5);
    CHECK_FLOATING_POINT(T, Angle::angle_interp(angle2, angle1, (T) 0.5), Angle::HalfPi<T>() * 0.5);

    // basic tests for other alphas
    CHECK_FLOATING_POINT(T, Angle::angle_interp(angle1, angle2, (T) 0.2), Angle::HalfPi<T>() * 0.2);
    CHECK_FLOATING_POINT(T, Angle::angle_interp(angle2, angle1, (T) 0.2), Angle::HalfPi<T>() * 0.8);

    // tests when angles straddle the 0/2pi crossing
    angle1 = -Angle::HalfPi<T>() / 2;
    angle2 = Angle::HalfPi<T>() / 2;
    CHECK_FLOATING_POINT(T, Angle::angle_interp(angle1, angle2, (T) 0.5), 0);
    CHECK_FLOATING_POINT(T, Angle::angle_interp(angle2, angle1, (T) 0.5), 0);


    CHECK_FLOATING_POINT(T, Angle::angle_interp(angle1, angle2, (T) 0.1), -0.8 * Angle::HalfPi<T>() / 2);
    CHECK_FLOATING_POINT(T, Angle::angle_interp(angle2, angle1, (T) 0.1), 0.8 * Angle::HalfPi<T>() / 2);

    // test when angles straddle pi
    angle1 = Angle::Pi<T>() - (Angle::HalfPi<T>() / 2);
    angle2 = Angle::Pi<T>() + (Angle::HalfPi<T>() / 2);
    ...
    angle1 = 0.4;
    angle2 = 0.8;
    CHECK_FLOATING_POINT(T, Angle::angle_interp(angle1, angle2, (T) 0), angle1);
    CHECK_FLOATING_POINT(T, Angle::angle_interp(angle1, angle2, (T) 1), angle2);
    CHECK_FLOATING_POINT(T, Angle::angle_interp(angle2, angle1, (T) 0), angle2);
    CHECK_FLOATING_POINT(T, Angle::angle_interp(angle2, angle1, (T) 1), angle1);

    // testing random angles
    angle1 = 0.2;
    angle2 = 0.8;
    CHECK_FLOATING_POINT(T, Angle::angle_interp(angle1, angle2, (T) 0.1), 0.26);
    CHECK_FLOATING_POINT(T, Angle::angle_interp(angle2, angle1, (T) 0.1), 0.74);

    // testing nonstandard inputs, alpha not in range [0, 1]
    CHECK_FLOATING_POINT(T, Angle::angle_interp(angle1, angle2, (T) 2.5), 1.7);
    CHECK_FLOATING_POINT(T, Angle::angle_interp(angle2, angle1, (T) 2.5), -0.7);

    CHECK_FLOATING_POINT(T, Angle::angle_interp(angle1, angle2, (T) -1.5), -0.7);
    CHECK_FLOATING_POINT(T, Angle::angle_interp(angle2, angle1, (T) -1.5), 1.7);
}
